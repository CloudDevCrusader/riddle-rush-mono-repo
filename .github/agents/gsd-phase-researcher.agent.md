...
```bash
INIT=$(node ".github/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node ".github/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
