...
async function cmdScanSessions(overridePath, options, raw) {
  ...
  if (!sessionsDir) {
    const searchedPath = overridePath || '.github/projects';
    error(
  ...
  process.stderr.write(
    'Reading your session history (read-only, nothing is modified or sent anywhere)...\n'
  );
