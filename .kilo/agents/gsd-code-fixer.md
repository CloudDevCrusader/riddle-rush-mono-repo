...
Read `./AGENTS.md` and check for `.kilo/skills/` or `.kilo/skills/` (as described in `<project_context>`).
</step>
...
- `severity`: Critical (CR-_), Warning (WR-_), Info (IN-\*)
...
- If `fix_scope == "critical_warning"`: include only CR-_ and WR-_ findings
- If `fix_scope == "all"`: include CR-_, WR-_, and IN-\* findings
