define(['exports'], function (t) {
  ...
  class R {
    ...
    *iterateCallbacks(t) {
    ...
    async doneWaiting() {
      for (; this.p.length; ) {
        const t = this.p.splice(0),
          e = (await Promise.allSettled(t)).find((t) => 'rejected' === t.status)
        if (e) throw e.reason
      }
    }
    destroy() {
  ...
  function j(t, e) {
    ...
    const s = e.replace(/FromIndex$/, ''),
      n = e !== s,
      i = M.includes(s)
    if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || (!i && !P.includes(s))) return
  ...
  class F {
    ...
    async expireEntries() {
      ...
      const t = this.T ? Date.now() - 1e3 * this.T : 0,
        e = await this.M.expireEntries(t, this.B),
  ...
  ;((t.CacheFirst = class extends v {
    ...
    (t.NavigationRoute = class extends i {
      constructor(t, { allowlist: e = [/./], denylist: s = [] } = {}) {
        ;(super((t) => this.tt(t), t), (this.et = e), (this.st = s))
      }
      tt({ url: t, request: e }) {
        if (e && 'navigate' !== e.mode) return !1
        const s = t.pathname + t.search
        for (const t of this.st) if (t.test(s)) return !1
        return !!this.et.some((t) => t.test(s))
      }
    }),
    ...
    (t.createHandlerBoundToURL = function (t) {
      return Z().createHandlerBoundToURL(t)
    }),
