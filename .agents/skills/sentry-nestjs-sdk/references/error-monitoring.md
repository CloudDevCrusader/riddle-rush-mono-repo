...
Sentry.captureMessage('Cache miss rate above threshold', {
  ...
  extra: { missRate: 0.42, threshold: 0.2 },
})
```
...
Sentry.init({
  beforeBreadcrumb(breadcrumb, hint) {
    ...
    if (breadcrumb.category === 'db.query' && breadcrumb.message?.includes('SELECT 1')) {
      return null
    }
...
```typescript
Sentry.init({
  ignoreErrors: ['ECONNRESET', /^Connection refused$/i, /^ETIMEDOUT/],
})
```
...
Sentry.init({
  beforeSend(event, hint) {
    ...
    if (error instanceof ExternalApiError) {
      event.fingerprint = ['{{ default }}', error.serviceName, String(error.statusCode)]
    }
