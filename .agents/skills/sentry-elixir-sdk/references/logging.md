...
1. **`Sentry.LoggerHandler`** — Erlang `:logger` handler that forwards crash reports and error log messages from your app to Sentry as _error events_. This is the primary way to catch errors that weren't explicitly passed to `capture_exception/2`.
