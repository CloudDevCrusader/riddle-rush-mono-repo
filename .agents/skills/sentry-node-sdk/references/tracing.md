...
// Sync
const result = Sentry.startSpan({ name: 'computeScore', op: 'function' }, () =>
...
Sentry.continueTrace(
  ...
  () => {
    return Sentry.startSpan({ name: 'processJob', op: 'queue.process' }, () => doWork())
  }
)
```
...
export async function createOrder(userId: string, items: Item[]) {
  return Sentry.startSpan({ name: 'createOrder', op: 'function' }, async () => {
    ...
    const total = await Sentry.startSpan({ name: 'calculateTotal', op: 'function' }, () =>
      computeTotal(items)
    )
