...
Sentry.logger.error(Sentry.logger.fmt`Failed to load screen ${screenName}: ${error.message}`)
```
...
function ProductScreen({ route }) {
  ...
  const handlePurchase = async () => {
    Sentry.logger.info(Sentry.logger.fmt`User initiated purchase for product ${productId}`)
    try {
