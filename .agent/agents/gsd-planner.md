...
tools: read_file, write_file, run_shell_command, glob, search_file_content, web_fetch
color: green
---
...
```bash
INIT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
````
...
```bash
STRUCTURE=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
