...
counts by running Glob on canonical locations above, not from memory or GEMINI.md.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node .agent/get-shit-done/bin/gsd-tools.cjs intel extract-exports <file>` to get accurate exports.
...
Run: `node .agent/get-shit-done/bin/gsd-tools.cjs intel validate --cwd <project_root>`
...
Run: `node .agent/get-shit-done/bin/gsd-tools.cjs intel snapshot --cwd <project_root>`
