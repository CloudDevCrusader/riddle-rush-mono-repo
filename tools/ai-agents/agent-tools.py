...
def run_command(
    cmd: str, cwd: Optional[str] = None, timeout: int = 60
...
def get_ai_agent_status() -> Dict[str, Union[str, bool, List[str]]]:
    ...
    # Check for LangChain
    langchain_check = run_command(
        ["python", "-c", "import langchain; print('langchain_ok')"]
    )
    ...
    if "fastmcp" in status["tools_installed"]:
        status["recommendations"].append(
            "Start FastMCP server for multi-agent coordination"
        )

    if "langchain" in status["tools_installed"]:
        status["recommendations"].append(
            "Use LangChain for AI-powered workflow automation"
        )

    if "trunk" in status["tools_installed"]:
        status["recommendations"].append(
            "Run 'pnpm run workspace:check' for comprehensive quality checks"
        )
...
def analyze_workspace_health() -> Dict[str, Union[str, int, List[Dict[str, str]]]]:
    ...
    build_exists = (PROJECT_ROOT / "dist").exists() or (
        PROJECT_ROOT / "apps" / "game" / ".output"
    ).exists()
    if build_exists:
        health_report["health_indicators"].append(
            {"type": "build", "status": "ok", "message": "Build artifacts found"}
        )
    else:
        health_report["issues_found"].append(
            {
    ...
    if not health_report["issues_found"]:
        ...
        health_report["recommendations"].append(
            "Address minor issues to improve workspace health"
        )
    else:
        health_report["overall_score"] = 60
        health_report["recommendations"].append(
            "Multiple issues detected - review and fix for optimal performance"
        )
