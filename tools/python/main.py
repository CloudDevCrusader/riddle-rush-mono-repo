...
def run_prettier_fix() -> str:
    ...
    try:
        ...
        return "❌ Prettier fix timed out"
    except Exception as e:
        return f"❌ Error running prettier fix: {str(e)}"
...
def aws_deploy(environment: str, skip_tests: bool = False) -> str:
    ...
    """
    try:
        script_map = {"production": "deploy-prod.sh", "development": "deploy-dev.sh"}
        ...
        result = subprocess.run(["bash", "-c", cmd], capture_output=True, text=True, timeout=600)  # 10 minutes
