...
`
    ...
    const result = runGsdTools(
      'summary-extract .planning/phases/01-foundation/01-01-SUMMARY.md',
      tmpDir
    )
    assert.ok(result.success, `Command failed: ${result.error}`)
...
`
    ...
    const result = runGsdTools(
      'summary-extract .planning/phases/01-foundation/01-01-SUMMARY.md --fields one_liner,key_files',
      tmpDir
    )
    assert.ok(result.success, `Command failed: ${result.error}`)
...
describe('init commands with --include flag', () => {
  ...
  test('init plan-phase includes multiple file contents', () => {
    ...
    const result = runGsdTools(
      'init plan-phase 03 --include state,roadmap,requirements,context,research',
      tmpDir
    )
    assert.ok(result.success, `Command failed: ${result.error}`)
  ...
  test('init plan-phase includes verification and uat content', () => {
    ...
    assert.ok(
      output.verification_content.includes('Verification Results'),
      'verification content correct'
    )
  ...
  test('partial includes work correctly', () => {
    ...
    assert.ok(output.state_content, 'state_content included')
    assert.strictEqual(
      output.roadmap_content,
      undefined,
      'roadmap_content not requested, should be undefined'
    )
  })
})
