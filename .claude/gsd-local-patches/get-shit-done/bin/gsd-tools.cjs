...
function isGitIgnored(cwd, targetPath) {
  try {
    execSync('git check-ignore -q -- ' + targetPath.replace(/[^\w.\-/]/g, ''), {
      cwd,
      stdio: 'pipe',
    })
    return true
...
function output(result, raw, rawValue) {
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      const tmpPath = path.join(require('node:os').tmpdir(), `gsd-${Date.now()}.json`)
...
function cmdStateAdvancePlan(cwd, raw) {
  ...
  if (!fs.existsSync(statePath)) {
    output({ error: 'STATE.md not found' }, raw); return
  }
...
function cmdStateAddDecision(cwd, options, raw) {
  ...
  if (!summary) {
    output({ error: 'summary required' }, raw); return
  }
  ...
  if (match) {
    ...
    // Remove placeholders
    sectionBody = sectionBody.replace(/None yet\.?\s*/gi, '').replace(/No decisions yet\.?\s*/gi, '')
...
function cmdStateAddBlocker(cwd, text, raw) {
  ...
  if (!text) {
    output({ error: 'text required' }, raw); return
  }
  ...
  if (match) {
    let sectionBody = match[2]
    sectionBody = sectionBody.replace(/None\.?\s*/gi, '').replace(/None yet\.?\s*/gi, '')
    sectionBody = sectionBody.trimEnd() + '\n' + entry + '\n'
    content = content.replace(sectionPattern, `${match[1]}${sectionBody}`)
    fs.writeFileSync(statePath, content, 'utf-8')
...
function cmdVerifySummary(cwd, summaryPath, checkFileCount, raw) {
  ...
  const patterns = [
    /`([^`]+\.[a-z]+)`/gi,
    /(?:Created|Modified|Added|Updated|Edited):\s*`?([^\s`]+\.[a-z]+)`?/gi,
  ]
...
function cmdTemplateFill(cwd, templateType, options, raw) {
  ...
  if (!phaseInfo || !phaseInfo.found) {
    output({ error: 'Phase not found', phase: options.phase }, raw); return
  }
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  for (const planFile of planFiles) {
    ...
    const wave = Number.parseInt(fm.wave, 10) || 1
...
function cmdFrontmatterGet(cwd, filePath, field, raw) {
  ...
  if (field) {
    ...
    if (value === undefined) {
      output({ error: 'Field not found', field }, raw); return
    }
...
function cmdFrontmatterMerge(cwd, filePath, data, raw) {
  ...
  try {
    ...
    error('Invalid JSON for --data'); return
  }
...
function cmdVerifyPlanStructure(cwd, filePath, raw) {
  ...
  if (fm.wave && Number.parseInt(fm.wave) > 1 && (!fm.depends_on || (Array.isArray(fm.depends_on) && fm.depends_on.length === 0))) {
...
function cmdVerifyPhaseCompleteness(cwd, phase, raw) {
  ...
  try {
    ...
    output({ error: 'Cannot read phase directory' }, raw); return
  }
...
function cmdVerifyReferences(cwd, filePath, raw) {
  ...
  const atRefs = content.match(/@([^\s,)]+\/[^\s,)]+)/g) || []
  ...
  const backtickRefs = content.match(/`([^`][^/`]*\/[^`]+\.[a-z]{1,10})`/gi) || []
...
function cmdVerifyArtifacts(cwd, planFilePath, raw) {
  ...
  const content = safeReadFile(fullPath)
  if (!content) {
    output({ error: 'File not found', path: planFilePath }, raw); return
  }
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      if (dm) existingDecimals.push(Number.parseInt(dm[1], 10))
    }
  } catch {}
  ...
  if (isDecimal) {
    ...
    try {
      ...
      for (const dir of dirs) {
        ...
        if (dm && Number.parseInt(dm[1], 10) > removedDecimal) {
          toRename.push({ dir, oldDecimal: Number.parseInt(dm[1], 10), slug: dm[2] })
        }
      }
      ...
      for (const dir of dirs) {
        ...
        if (dirInt > removedInt) {
          toRename.push({
            dir,
            oldInt: dirInt,
            decimal: dm[2] ? Number.parseInt(dm[2], 10) : null,
            slug: dm[3],
          })
        }
      }
