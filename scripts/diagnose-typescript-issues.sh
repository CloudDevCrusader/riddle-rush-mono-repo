...
if grep -q "typescript@5.9" apps/game/.nuxt/tsconfig.app.json; then
    echo "❌ ISSUE FOUND: Nuxt is using TypeScript 5.9.x but project is configured for $root_ts_version"
...
if [ -f "apps/game/tsconfig.json" ]; then
    ...
    if grep -q "extends" apps/game/tsconfig.json; then
        ...
        for file in $extended_files; do
            if [ -f "$file" ]; then
                echo "   ✅ Extended file exists: $file"
            else
                echo "   ❌ Extended file missing: $file"
            fi
        done
    else
...
if [ -f ".vscode/settings.json" ]; then
    if grep -q "typescript.tsdk" ".vscode/settings.json"; then
        ...
        echo "✅ TypeScript SDK setting found: $tsdk_setting"
